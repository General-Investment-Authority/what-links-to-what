write_ISIC_to_RDF <- function(ws, version, dataDir, turtlePath){
  baseURL = paste0("http://isdata.org/Classifications/ISIC/", version, "/")
  ...
  save.rdf(ontStore, paste0(turtlePath, "/ISIC", version, ".turtle"), format="TURTLE")
}

parse_ISIC <- function(turtlePath = "./data/Turtle"){
  dir.create(turtlePath, recursive=TRUE)
  versions = get_classification_versions("ISIC")

  for (item in versions){
    dataDir = paste0("./data/ISIC/", item$version)
    ...
    write_ISIC_to_RDF(ws, item$version, dataDir, turtlePath)
  }
}
